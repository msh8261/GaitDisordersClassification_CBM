...
def _row_swap_cost(benefit_matrix, assignment, row_idx):
    ...
    """
    swap_cost = (
        benefit_matrix[row_idx, assignment] + benefit_matrix[:, assignment[row_idx]]
    )
    curr_cost = (
        benefit_matrix[row_idx, assignment[row_idx]]
...
def _best_row_swap(benefit_matrix, assignment):
    ...
    best_row, best_row_benefit = np.stack(
        [
            _row_swap_cost(benefit_matrix, assignment, r)
            for r in np.arange(assignment.shape[0])
        ]
    ).T
...
def _row_swap(
    benefit_matrix, assignment, best_row, br_benefit, best_col, bc_benefit, r_idx
):
...
def asymmetric_greedy_search(benefit_matrix, shuffle=False, minimize=False):
    ...
    while brb_max > 0 or bcb_max > 0:
        while brb_max > 0 or bcb_max > 0:
            if brb_max > bcb_max:
                r = np.argmax(brb)
                assignment, brs, brb, bcs, bcb = _row_swap(
                    bm, assignment, brs, brb, bcs, bcb, r
                )
            else:
                r = np.argmax(bcb)
                assignment, brs, brb, bcs, bcb = _col_swap(
                    bm, assignment, brs, brb, bcs, bcb, r
                )
            brb_max = np.amax(brb)
            bcb_max = np.amax(bcb)
