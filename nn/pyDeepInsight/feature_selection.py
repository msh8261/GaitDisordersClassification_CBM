...
class CAMFeatureSelector:
    ...
    def _resolve_target_layer(
        self, target_layer: Union[str, nn.Module] = None
    ) -> Optional[nn.Module]:
    ...
    def compute_cam(
        ...
        for i, data in enumerate(dl):
            ...
            inputs, labels = data
            cam = func(
                model=self.model, target_layers=[self.target_layer], use_cuda=use_cuda
            )
    ...
    @staticmethod
    def flatten_classes(
        labels: np.ndarray, cam: np.ndarray, method: str = "mean"
    ) -> Dict[int, np.ndarray]:
        ...
        for cat in cats:
            ...
            flat_act = CAMFeatureSelector.flatten_cam(cams=cat_act, method=method)
            cat_cam[cat] = flat_act
        return cat_cam

    def calculate_class_activations(
        self, X: Tensor, y: Tensor, batch_size: int = 1, flatten_method="mean"
    ) -> Dict[int, np.ndarray]:
        ...
        """
        use_cuda = X.is_cuda
        activations = self.compute_cam(
            X=X, y=y, batch_size=batch_size, use_cuda=use_cuda
        )
    ...
    def select_class_features(
        self, cams: Dict[int, np.ndarray], threshold: float = 0.6
    ) -> Dict[int, np.ndarray]:
