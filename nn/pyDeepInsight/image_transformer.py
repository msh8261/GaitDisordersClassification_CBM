...
class ManifoldLearner(Protocol):
    def fit_transform(self: "ManifoldLearner", X: np.ndarray) -> np.ndarray:
        pass
...
class ImageTransformer:
    ...
    def __init__(
        self,
        feature_extractor: Union[str, ManifoldLearner] = "tsne",
    ...
    def _parse_feature_extractor(
        ...
        if isinstance(feature_extractor, str):
            ...
            if fe == "tsne".casefold():
                ...
                raise ValueError(f"feature_extractor '{feature_extractor}' not valid")
    ...
    @classmethod
    def coordinate_binning(
        cls, position: np.ndarray, px_size: Tuple[int, int]
    ) -> np.ndarray:
    ...
    def lsap_heuristic_solution(cost_matrix):
        return asymmetric_greedy_search(cost_matrix, shuffle=True, minimize=True)

    @classmethod
    def coordinate_optimal_assignment(
        cls, position: np.ndarray, px_size: Tuple[int, int]
    ) -> np.ndarray:
    ...
    @classmethod
    def coordinate_heuristic_assignment(
        cls, position: np.ndarray, px_size: Tuple[int, int]
    ) -> np.ndarray:
    ...
    def fit(self, X: np.ndarray, y: Optional[ArrayLike] = None, plot: bool = False):
    ...
    def transform(
        self, X: np.ndarray, img_format: str = "rgb", empty_value: int = 0
    ) -> np.ndarray:
        ...
        """
        img_coords = (
            pd.DataFrame(np.vstack((self._coords.T, X)).T)
        ...
        for z in range(2, img_coords.shape[1]):
            ...
            img_matrix[img_coords[0].astype(int), img_coords[1].astype(int)] = (
                img_coords[z]
            )
    ...
    def inverse_transform(self, img: np.ndarray) -> np.ndarray:
        ...
        if img.ndim == 2 and img.shape == self._pixels:
            ...
            raise ValueError(
                (
                    f"Expected dimensions of (B, {self._pixels[0]}, "
