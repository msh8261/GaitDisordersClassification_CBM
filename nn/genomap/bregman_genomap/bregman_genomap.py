...
def greenkhorn(
    a, b, M, reg, numItermax=10000, stopThr=1e-9, verbose=False, log=False, warn=True
):
    r"""
    Solve the entropic regularization optimal transport problem and return the OT matrix
    ...
    **kwargs
    ...
    for ii in range(numItermax):
        ...
        if nx.max(nx.abs(u)) > tau or nx.max(nx.abs(v)) > tau:
            if n_hists:
                alpha, beta = alpha + reg * nx.max(nx.log(u), 1), beta + reg * nx.max(
                    nx.log(v)
                )
            else:
...
def barycenter(
    ...
    **kwargs
    ...
    if method.lower() == "sinkhorn":
        ...
        return _barycenter_sinkhorn_log(
            A,
            M,
            reg,
            weights=weights,
...
def barycenter_debiased(
    ...
    **kwargs
    ...
    if method.lower() == "sinkhorn":
        ...
        return _barycenter_debiased_log(
            A,
            M,
            reg,
            weights=weights,
