...
def autoencoder(dims, act="relu"):
    ...
    return Model(inputs=x, outputs=y, name="AE"), Model(
...
class FcDEC(object):
                ...
                def on_epoch_end(self, epoch, logs=None):
                    ...
                    feature_model = Model(
                        ...
                        self.model.get_layer(
                            index=int(len(self.model.layers) / 2)
                        ).output,
                    )
        ...
        if not aug_pretrain:
            ...
            def gen(x, batch_size):
                if len(x.shape) > 2:  # image
                    ...
                    gen0 = self.datagen.flow(
                        np.reshape(x, im_shape), shuffle=True, batch_size=batch_size
                    )
                    while True:
                        batch_x = gen0.next()
    ...
    def random_transform(self, x):
        ...
        if width * width == x.shape[-1]:  # gray
            ...
            width = int(np.sqrt(x.shape[-1] / 3.0))
            im_shape = [-1, width, width, 3]
        gen = self.datagen.flow(
            np.reshape(x, im_shape), shuffle=False, batch_size=x.shape[0]
        )
