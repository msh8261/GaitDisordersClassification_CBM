...
from genomap.utils.gTraj_utils import (compute_cluster_distances,
                                       nearest_divisible_by_four)
...
def genoMOIvis(
    *arrays,
    ...
    if prealign_method == "scanorama":
        batch_corrected_data = apply_scanorama_and_return_batch_corrected(
            *arrays, n_dim=colNum * rowNum
        )
    else:
    ...
    embedding, y_pred, feat_DNN = extract_genoVis_features(
        dataDX,
...
def extract_genoVis_features(
    ...
    optimizer = Adam()
    model = ConvIDEC(
        input_shape=genoMaps.shape[1:],
        filters=[32, 64, 128, n_dim],
        n_clusters=n_clusters,
    )
    ...
    embedding2D = umap.UMAP(n_neighbors=30, min_dist=0.3, n_epochs=200).fit_transform(
        feat_DNN
    )
...
def apply_bbknn_and_return_batch_corrected(*arrays):
    ...
    sc.external.pp.bbknn(
        merged_adata, batch_key="batch", neighbors_within_batch=5, n_pcs=50
    )
