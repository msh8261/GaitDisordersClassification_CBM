...
def sctype_score(
    ...
    # Marker sensitivity
    marker_stat = (
        pd.Series([gene for sublist in gs for gene in sublist])
        .value_counts()
    ...
    gs2 = [
        [gene for gene in sublist if gene in scRNAseqData.columns] for sublist in gs2
    ]

    cell_markers_genes_score = marker_sensitivity[
        marker_sensitivity["gene_"].isin(np.unique(np.concatenate(gs)))
    ]
...
def gene_sets_prepare(path_to_db_file, cell_type, species="human"):
    ...
    cell_markers["geneSymbolmore1"] = (
        cell_markers["geneSymbolmore1"].astype(str).str.replace(" ", "")
    )
    cell_markers["geneSymbolmore2"] = (
        cell_markers["geneSymbolmore2"].astype(str).str.replace(" ", "")
    )

    cell_markers["geneSymbolmore1"] = cell_markers["geneSymbolmore1"].apply(
        lambda x: correct_gene_symbols(x, species=species)
    )
    cell_markers["geneSymbolmore2"] = cell_markers["geneSymbolmore2"].apply(
        lambda x: correct_gene_symbols(x, species=species)
    )
...
def correct_gene_symbols(gene_symbols, species="human"):
    ...
    gene_symbols = [
        gs.strip().replace(" ", "").upper()
        for gs in gene_symbols
        if gs.strip().upper() not in ["NA", ""]
    ]
    ...
    if len(gene_symbols) > 0:
        ...
        suppressMessages = (
            lambda x: x
        )  # Suppressing messages in Python is not necessary
...
def check_gene_symbols(x, unmapped_as_na=True, map=None, species="human"):
    ...
    if species == "human":
        ...
        if map is None:
            raise ValueError(
                "If species is not 'human' or 'mouse', then map argument must be specified"
            )

    if not isinstance(map, pd.DataFrame) or not set(map.columns) == {
        "Gene name",
        "Gene type",
    }:
        raise ValueError(
            "If map is specified, it must be a dataframe with two columns named 'external_gene_name' and 'gene_biotype'"
        )
    ...
    if casecorrection:
        ...
        x_casecorrected = [
            re.sub(r"(.*C[0-9XY]+)ORF(.+)", r"\1orf\2", sym) for sym in x_casecorrected
        ]
    else:
        x_casecorrected = x.split()

    approvedaftercasecorrection = [
        sym in map["Gene name"].values for sym in x_casecorrected
    ]
        ...
        if approved[i]:
            ...
            if alias[i]:
                suggested_symbols.append(
                    " /// ".join(
                        map.loc[map["Gene name"] == x_casecorrected[i], "Gene name"]
                    )
                )
    ...
    df = pd.DataFrame(
        {"x": x, "Approved": approved, "Suggested.Symbol": suggested_symbols}
    )
    df["Approved"][df["x"].isnull()] = False

    if not unmapped_as_na:
        df.loc[df["Suggested.Symbol"].isnull(), "Suggested.Symbol"] = df.loc[
            df["Suggested.Symbol"].isnull(), "x"
        ]
