...
def compute_genoSig(imageset, label, sig_class, epochs=100):
    ...
    model.add(
        Conv2D(
            ...
            kernel_size=(3, 3),
            activation="relu",
            input_shape=(imageset.shape[1], imageset.shape[2], imageset.shape[3]),
        )
    )
    ...
    for i in range(imgs.shape[0]):
        ...
        explanation = explainer.explain_instance(
            image, model.predict, top_labels=n_clusters, hide_color=0, num_samples=100
        )
        ...
        lime_img, mask = explanation.get_image_and_mask(
            ground_truth,
            positive_only=False,
            num_features=imgs.shape[1] * imgs.shape[2],
            hide_rest=False,
        )
...
def genoSig(genoMaps, T, label, userPD, gene_names, epochs=100):
    ...
    for ii in range(0, len(meanI)):
        meanIX = meanI[ii]
        Ivec = np.reshape(
            meanIX, (1, meanIX.shape[0] * meanIX.shape[1]), order="F"
        ).copy()
