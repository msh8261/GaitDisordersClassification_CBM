...
def save_accuracy_in_file(labels_gt, labels):
    with open(
        "results/test_results_gait2_cls" + str(config.params["num_class"]) + ".txt", "w"
    ) as f:
        ...
        pp_matrix(
            ...
            lw=0.5,
            figsize=[8, 6],
            save_file_name="results/metrics_confusion.png",
        )
        ...
        save_file_name = "results/precision_recall.png"
        plot_precision_recall(
            recall, precision, fscore, average_precision, save_file_name
        )


def test_saved_model(path_model, model_name, test_dataset):
...
if __name__ == "__main__":
    ...
    for file_name in os.listdir(dir_path):
        ...
        test_saved_model(path_model, model_name, test_dataset)
