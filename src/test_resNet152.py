...
def save_accuracy_in_file(labels_gt, labels):
    with open(
        "results/test_results_resnet152_cls" + str(config.params["num_class"]) + ".txt",
        "w",
    ) as f:
        ...
        pp_matrix(
            ...
            lw=0.5,
            figsize=[8, 6],
            save_file_name="results/resnet152_metrics_confusion.png",
        )
        ...
        save_file_name = "results/resnet152_precision_recall.png"
        plot_precision_recall(
            recall, precision, fscore, average_precision, save_file_name
        )


def transform_data(X_test, y_test, pixel_size):
    ln = Norm2Scaler()
    ...
    it.fit(X_test, y=y_test, plot=True)
...
def test_saved_model(path_model, model_name, X_test, y_test):
    ...
    X_test = transform_data(X_test, y_test, (70, 70))
...
if __name__ == "__main__":
    ...
    # path_model = r"results\deepInsight\152\deepInsight_k0_rand21.pt"
    ...
    for file_name in os.listdir(dir_path):
        ...
        test_saved_model(path_model, model_name, X_test, y_test)
