...
def save_accuracy_in_file(labels_gt, labels):
    with open(
        "results/test_results_genoNet_cls" + str(config.params["num_class"]) + ".txt",
        "w",
    ) as f:
        ...
        pp_matrix(
            ...
            lw=0.5,
            figsize=[8, 6],
            save_file_name="results/genoNet_metrics_confusion.png",
        )
        ...
        save_file_name = "results/genoNet_precision_recall.png"
...
def test_saved_model(path_model, model_name, X_test, y_test, pixe_size):
    ...
    X_test = get_genomap_data(X_test, pixe_size)
